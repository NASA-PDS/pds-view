...
import gov.nasa.pds.objectAccess.LabelParser;
...
public class LabelParserTest {
	...
	public void testFlatten() throws Exception {
		...
		Assert.assertEquals(818, v.length());
		Assert.assertEquals("Product_Bundle", transformXML.getTitle());
		Assert.assertEquals(11, countEquals(v));
		Assert.assertEquals(10, countBreaks(v, sep));
	}
	...
	public void testTokenize() throws Exception {
		...
		String v = lp.flatten();
		Assert.assertEquals("Product_Array_2D_Image", lp.getTitle());
		Assert.assertEquals(60, countEquals(v));  //number of flattened lines
		Assert.assertEquals(59, countBreaks(v, separator));
		
		TokenizedLabel tokenizedLabel = lp.tokenize(v);
		Assert.assertEquals(expectedTitle, tokenizedLabel.getTitle());		
		Assert.assertTrue(tokenizedLabel.getAllSections().containsKey("Identification_Area_Product"));
		TokenizedLabelSection idArea = tokenizedLabel.getSectionByTitle("Identification_Area_Product");
		Assert.assertEquals(10, idArea.getAllNameValuePairs().size());
		
		Map<String, TokenizedLabelSection> allSections = tokenizedLabel.getAllSections();
		Assert.assertEquals(4, allSections.size());
		TokenizedLabelSection fileObservationalArea = tokenizedLabel.getSectionByTitle("File_Area_Observational");
		Assert.assertEquals( "File.local_identifier", fileObservationalArea.getAllNameValuePairs().get(0)[0]);
		Assert.assertEquals( "i943630r.raw", fileObservationalArea.getAllNameValuePairs().get(0)[1]);		
	}
