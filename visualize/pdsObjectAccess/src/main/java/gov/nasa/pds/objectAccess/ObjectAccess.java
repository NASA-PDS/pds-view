...
public class ObjectAccess implements ObjectProvider {
	Logger logger = LoggerFactory.getLogger(ObjectAccess.class);
	private String archiveRoot;
	
	public ObjectAccess(String archiveRoot) throws Exception {
	...
	public DocumentProduct parseDocumentProduct(String thisRelativeFilename) throws Exception {
		...
		try {
			...
			for (int k=0; k< formatSetXML.getLength(); k++) {
				...
				// ... and its mandatory attrs				
				NodeList es = documentFile.getElementsByTagName("external_standard_id");
				String esid = getCharacterDataFromElement((Element)es.item(0));				
