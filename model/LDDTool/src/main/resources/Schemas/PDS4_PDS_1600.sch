<?xml version="1.0" encoding="UTF-8"?>
  <!-- PDS4 Schematron for Name Space Id:pds  Version:1.6.0.0 - Thu Mar 17 10:36:33 PDT 2016 -->
...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    <sch:rule context="pds:Bundle_Member_Entry">
      ...
      <sch:assert test="if (pds:lid_reference) then starts-with(pds:lid_reference, $urn_nasa) or starts-with(pds:lid_reference, $urn_esa) or starts-with(pds:lid_reference, $urn_ros) else true()">
        The value of the attribute lid_reference must start with either: <sch:value-of select="$urn_nasa"/> or <sch:value-of select="$urn_esa"/> or <sch:value-of select="$urn_ros"/></sch:assert>
      <sch:assert test="if (pds:lidvid_reference) then starts-with(pds:lidvid_reference, $urn_nasa) or starts-with(pds:lidvid_reference, $urn_esa) or starts-with(pds:lidvid_reference, $urn_ros) else true()">
        The value of the attribute lidvid_reference must start with either: <sch:value-of select="$urn_nasa"/> or <sch:value-of select="$urn_esa"/> or <sch:value-of select="$urn_ros"/></sch:assert>
  ...
  <sch:pattern>
    <sch:rule context="pds:Encoded_Image" role="warning">
      <sch:assert test="pds:encoding_standard_id != 'J2C'">
        The value J2C for attribute Encoded_Image.encoding_standard_id is deprecated and should not be used.</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
  ...
  <sch:pattern>
    <sch:rule context="pds:Field_Binary">
      <sch:let name="BitLength_1" value="pds:data_type = ('SignedByte', 'UnsignedByte')"/>
      <sch:let name="BitLength_2" value="pds:data_type = ('SignedLSB2', 'SignedMSB2', 'UnsignedLSB2', 'UnsignedMSB2')"/>
      <sch:let name="BitLength_4" value="pds:data_type = ('IEEE754LSBSingle', 'IEEE754MSBSingle', 'SignedLSB4', 'SignedMSB4', 'UnsignedLSB4', 'UnsignedMSB4')"/>
      <sch:let name="BitLength_8" value="pds:data_type = ('ComplexLSB8', 'ComplexMSB8', 'IEEE754LSBDouble', 'IEEE754MSBDouble', 'SignedLSB8', 'SignedMSB8', 'UnsignedLSB8', 'UnsignedMSB8')"/>
      <sch:let name="BitLength_16" value="pds:data_type = ('ComplexLSB16', 'ComplexMSB16')"/>
      <sch:assert test="if ($BitLength_1) then (pds:field_length = 1) else true()">
        The attribute pds:field_length ('<sch:value-of select="pds:field_length" />') does not match pds:data_type ('<sch:value-of select="pds:data_type" />').</sch:assert>
      <sch:assert test="if ($BitLength_2) then (pds:field_length = 2) else true()">
        The attribute pds:field_length ('<sch:value-of select="pds:field_length" />') does not match pds:data_type ('<sch:value-of select="pds:data_type" />').</sch:assert>
      <sch:assert test="if ($BitLength_4) then (pds:field_length = 4) else true()">
        The attribute pds:field_length ('<sch:value-of select="pds:field_length" />') does not match pds:data_type ('<sch:value-of select="pds:data_type" />').</sch:assert>
      <sch:assert test="if ($BitLength_8) then (pds:field_length = 8) else true()">
        The attribute pds:field_length ('<sch:value-of select="pds:field_length" />') does not match pds:data_type ('<sch:value-of select="pds:data_type" />').</sch:assert>
      <sch:assert test="if ($BitLength_16) then (pds:field_length = 16) else true()">
        The attribute pds:field_length ('<sch:value-of select="pds:field_length" />') does not match pds:data_type ('<sch:value-of select="pds:data_type" />').</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
  ...
  <sch:pattern>
    <sch:rule context="pds:Identification_Area">
      ...
      <sch:assert test="if (pds:logical_identifier) then starts-with(pds:logical_identifier, $urn_nasa) or starts-with(pds:logical_identifier, $urn_esa) or starts-with(pds:logical_identifier, $urn_ros) else true()">
        The value of the attribute logical_identifier must start with either: <sch:value-of select="$urn_nasa"/> or <sch:value-of select="$urn_esa"/> or <sch:value-of select="$urn_ros"/></sch:assert>
  ...
  <sch:pattern>
    <sch:rule context="pds:Identification_Area/pds:information_model_version">
      <sch:assert test=". = ('1.6.0.0')">
        The attribute pds:information_model_version must be equal to the value '1.6.0.0'.</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
  ...
  <sch:pattern>
    <sch:rule context="pds:Observing_System_Component/pds:type">
      <sch:assert test=". = ('Artificial Illumination', 'Facility', 'Instrument', 'Laboratory', 'Literature Search', 'Naked Eye', 'Observatory', 'Spacecraft', 'Telescope')">
        The attribute pds:type must be equal to one of the following values 'Artificial Illumination', 'Facility', 'Instrument', 'Laboratory', 'Literature Search', 'Naked Eye', 'Observatory', 'Spacecraft', 'Telescope'.</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
  ...
  <sch:pattern>
    <sch:rule context="pds:Property_Maps/pds:property_map_type">
  ...
  <sch:pattern>
    <sch:rule context="pds:Target/pds:type">
      <sch:assert test=". = ('Asteroid', 'Calibration', 'Comet', 'Dust', 'Dwarf Planet', 'Equipment', 'Galaxy', 'Globular Cluster', 'Lunar Sample', 'Meteorite', 'Meteoroid', 'Meteoroid Stream', 'Nebula', 'Open Cluster', 'Planet', 'Planetary Nebula', 'Planetary System', 'Plasma Cloud', 'Plasma Stream', 'Ring', 'Satellite', 'Star', 'Star Cluster', 'Sun', 'Synthetic Sample', 'Terrestrial Sample', 'Trans-Neptunian Object')">
