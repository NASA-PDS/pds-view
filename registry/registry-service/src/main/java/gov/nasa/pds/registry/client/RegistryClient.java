...
public class RegistryClient {
	...
	public ClientResponse publishProduct(String user, Product product) {
		WebResource.Builder builder = registryResource.path("products").getRequestBuilder();
		if (token != null) {
			builder = builder.header("Cookie", "iPlanetDirectoryPro=\"" + token
					+ "\"");
		}
		return builder.accept(mediaType).post(
				ClientResponse.class, product);
	}
	...
	public ClientResponse versionProduct(String user, Product product,
		...
		WebResource.Builder builder = registryResource.path("products").path(product.getLid()).queryParam("major", major.toString()).getRequestBuilder();
		if (token != null) {
			builder = builder.header("Cookie", "iPlanetDirectoryPro=\"" + token
					+ "\"");
		}
	...
	public ClientResponse getLatestProduct(String lid) {
		WebResource.Builder builder = registryResource.path("products").path(lid).getRequestBuilder();
		if (token != null) {
			builder = builder.header("Cookie", "iPlanetDirectoryPro=\"" + token
					+ "\"");
		}
		return builder.accept(mediaType)
				.get(ClientResponse.class);
	}

	public ClientResponse getProduct(String lid, String version) {
		WebResource.Builder builder = registryResource.path("products").path(lid).path(version).getRequestBuilder();
		if (token != null) {
			builder = builder.header("Cookie", "iPlanetDirectoryPro=\"" + token
					+ "\"");
		}
	...
	public ClientResponse getStatus() {
		WebResource.Builder builder = registryResource.path("status").getRequestBuilder();
		if (token != null) {
			builder = builder.header("Cookie", "iPlanetDirectoryPro=\"" + token
					+ "\"");
		}
		return builder.accept(mediaType).get(
				ClientResponse.class);
	}

	public ClientResponse publishAssociation(String user,
			Association association) {
		WebResource.Builder builder = registryResource.path("associations").getRequestBuilder();
		if (token != null) {
			builder = builder.header("Cookie", "iPlanetDirectoryPro=\"" + token
					+ "\"");
		}
		return builder.accept(mediaType).post(
				ClientResponse.class, association);
	}
	...
	public ClientResponse getProducts(ProductQuery query, Integer start, Integer rows) {
