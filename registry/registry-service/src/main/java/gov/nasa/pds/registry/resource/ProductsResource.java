...
public class ProductsResource {
	...
	/**
	 * Allows access to all the products managed by this repository. This list
	 * of products is based on the latest received product's logical identifier
	 ...
	 * @param objectType
	 *            filter to apply on the user defined registry object
	 *            types,supports wildcard (*)
	 * @param status
	 *            filter to apply on the object status, maps to
	 *            {@link ObjectStatus} enum
	 * @param queryOp
	 *            operator to apply to filters, valid values are AND or OR.
	 *            Defaults to AND.
	 * @param sort
	 *            defines what parameters to filter on. The format is
	 *            "parameter order" the order is optional. The default is
	 *            "guid ASC" and if unspecified the ordering is ASC.
	 * 
	...
	public Response getProducts(
		...
		ProductQuery.Builder queryBuilder = new ProductQuery.Builder().filter(filter).operator(operator);
		if (sort != null) {
			queryBuilder.sort(sort);
		}
		PagedResponse pr = registryService.getProducts(queryBuilder.build(),
				start, rows);
		...
		if (start > 1) {
			...
			if (previous < 1)
				previous = 1;
			String previousUri = absolute.clone().build(previous, rows)
					.toString();
	 ...
	 * notification. The submitted product should not contain the same logical
	 * identifier as previously submitted product (412 Precondition Failed), in
	 * that scenario the version interface should be used.
	 * 
	...
	public Response publishProduct(Product product) {
		...
		return Response
				.created(ProductResource.getProductUri(created, uriInfo))
				.build();
	}
	...
	@Path("{lid}")
	@Consumes( { MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON })
	public Response versionProduct(Product product,
		...
		// provided
		// TODO Change to set user
		Product created = registryService.versionProduct("Unknown", lid,
				product, major);
		return Response
				.created(ProductResource.getProductUri(created, uriInfo))
				.build();
	...
	/**
	 * Retrieves the collection of products that share the same local
	 * identifier. This method supports finding all versions of an product.
	 * 
	...
	/**
	 * Retrieves the latest product from the registry. The local identifier
	 * points to a collection of versions of the same product.
	 * 
	 * @response.representation.200.qname {http://registry.pds.nasa.gov}product
	 * @response.representation.200.mediaType application/xml
