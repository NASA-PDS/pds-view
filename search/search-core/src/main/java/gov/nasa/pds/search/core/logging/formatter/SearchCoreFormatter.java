...
public class SearchCoreFormatter extends Formatter {
  ...
  public SearchCoreFormatter() {
    this.config = new StringBuffer("PDS Search Core Tool Log" + doubleLineFeed);
  ...
  private void processSummary() {
    ...
    for (String key : SearchCoreStats.missingAssociations.keySet()) {
    	this.summary.append(key + lineFeed);
	    for (String slotName : SearchCoreStats.missingAssociations.get(key)) {
	    	this.summary.append("----- " + slotName
	          + lineFeed);
	    }
	    this.summary.append(lineFeed);
    ...
    for (String key : SearchCoreStats.missingAssociationTargets.keySet()) {
    	this.summary.append(key + lineFeed);
	    for (String slotName : SearchCoreStats.missingAssociationTargets.get(key)) {
	    	this.summary.append("----- " + slotName
	          + lineFeed);
	    }
	    this.summary.append(lineFeed);
