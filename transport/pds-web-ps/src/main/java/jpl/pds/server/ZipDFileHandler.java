...
/**
 * Handle product queries to zip directories of PDS products. If zipName is
 * PDS_ZIPD_SIZE then return the total size of files that PDS_ZIPD would
 * download.
 *
 * @author Crichton and Hughes.
 */
public class ZipDFileHandler extends ChunkedFileHandler {
	public XMLQuery queryForFile(XMLQuery query, File[] fileArray, String zipName) {
		boolean returnSize = zipName.equals("PDS_ZIPD_SIZE");
		int j;

		for (j=0; j<fileArray.length; j++) {
			if (!fileArray[j].isDirectory()) {
				System.err.println("ZipDFileHandler can't access dir " + fileArray[j]);
				return query;
			}
		...
		if (returnSize) {
			...
			if (!compatible) {
				System.err.println("ZipDFileHandler can't satisfy request for non-compatible MIME types");
				return query;
			}
			...
			for (j=0; j<fileArray.length; j++) {
				...
				filenames = PDS2FName.getDirectory(fileArray[j],filenames);
			}
			if (filenames == null) {
			...
			try {
				...
				String id = "zipd" + String.valueOf(productID++);
	...
	public static void main(String[] argv) {
		...
		try {
			...
			FileOutputStream dst = new FileOutputStream(ZIPDFILE);
	...
	private static final String ZIPDFILE = System.getProperty("jpl.pds.server.ZipDFileHandler.zipfile",
		"C:/temp/ZipDFile.zip");
}
