...
public class SFTPConnect implements RemoteFileTransfer {
	 ...
	 */
	private void connect(final String hostname, final String username,
			final String password) throws JSchException {
		JSch jsch = new JSch();

		this.session = jsch.getSession(username, hostname, 22);

		this.session.setConfig("StrictHostKeyChecking", "no"); // Ignore
																// HostKeyChecking
		this.session.setPassword(decrypt(password)); // set the password for
														// authentication
	 ...
	 * @return
	 */
	public final JsonObject checkConnection(final JsonObject start,
			final String hostname, final String username,
			final String password, final String pathname) {
		byte connect = 1;
		byte sftp = 1;
	 ...
	 */
	public final void getLogs(final String hostname, final String username,
			final String password, final String pathname,
		...
		try {
			...
			for (JsonElement element : array) {
				...
				if (!localFileList.contains(filename)) {
					this.log.info("Transferring - " + hostname + ": "
							+ basePath + "/" + filename + " to " + logDestPath
							+ "\n");
					this.sftpChannel
							.get(basePath + "/" + filename, logDestPath);
				} else {
					this.log.info(hostname + ": " + basePath + "/" + filename
							+ " already exists in " + logDestPath + "\n");
				}
			}
		} catch (JSchException e) {
			this.log.warning("Jsch Error: " + e.getMessage());
