...
AjaxSolr.DetailLinkSearchWidget = AjaxSolr.AbstractTextWidget.extend({
    ...
    setDetailDiv: function(id){
        ...
        $("#pbDetailContainer").empty();
        $("#pbDetailContainer").append(this.template(product));
    },

    setTitle:function(product){
            $("#pbDetailTitle").empty();
            if(product.person_sort_name !== undefined){
                $("#pbDetailTitle").append('<span style="font-size:175%;vertical-align: middle;">' + product.person_sort_name + ' &nbsp;</span><button id="returnToSearchButtonTop" type="button" class="btn btn-info btn-sm"><span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span> Return To Search</button>');
            }
            else{
                $("#pbDetailTitle").append('<span style="font-size:175%;vertical-align: middle;">' + product.person_sort_name + ' &nbsp;</span><button id="returnToSearchButtonTop" type="button" class="btn btn-info btn-sm"><span class="glyphicon glyphicon-arrow-left" aria-hidden="true"></span> Return To Search</button>');
            }
    ...
    template: function (product){
        ...
        //output += this.createListRow("E-Mail", product.person_electronic_mail_address);
        output += this.createRow("E-Mail", product.person_electronic_mail_address);
        output += this.createRow("Telephone Number", product.person_telephone_number);
        output += this.createRow("Alternate Number", product.person_alternate_telephone_number);
        output += this.createRow("Address", product.person_postal_address_text);
        output += this.createRow("Institution", product.person_institution_name);
        //output += this.createListRow("Node/Team", product.person_team_name);
        output += this.createRow("Node/Team", product.person_team_name);
        output += this.createRow("PDS Affiliation", product.person_affiliation_type);
        output += '</table></div>';
        return output;
    },
    ...
    createListRow: function(name, value){
        ...
        if(value !== undefined){
            ...
            output += this.arrayToHtmlList(value);
            output += '</td></tr>';
        }
        return output;
    },

    arrayToHtmlList: function(array){
        var output = "";

        if (array.length > 0){
            output += "<ul style='list-style-type: none;padding-left:0;'>";
            for (var i = 0; i < array.length; i++){
                console.log("arrayElem", array[0]);
