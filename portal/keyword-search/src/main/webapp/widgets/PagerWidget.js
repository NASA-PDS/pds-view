...
AjaxSolr.PagerWidget = AjaxSolr.AbstractWidget.extend(
  ...
  gapMarker: function () {
    ...
    //return $('<span class="pager-gap"></span>').html('&hellip;');
    return $('<span class="pager-gap">&hellip;</span>');
  },
  ...
  clickHandler: function (page) {
    var self = this;
    return function () {
      self.manager.store.manager.response.servlet = "";
        self.manager.store.manager.servlet = "";

      console.log("manageronpager:", self.manager.store);
  ...
  renderLinks: function (links) {
    if (this.totalPages) {
      ...
      for (var i = 0, l = links.length; i < l; i++) {
        var $li = $('<li></li>');
        if(links[i].attr("class") != undefined){
          if (links[i].attr("class").indexOf("pager-current") > -1){
            $li = $('<li class="active"></li>');
          }
          else if (links[i].attr("class").indexOf("pager-disabled") > -1){
   ...
   */
  perPage: function () {
    return parseInt(this.manager.response.responseHeader && this.manager.response.responseHeader.params && this.manager.response.responseHeader.params.rows || this.manager.store.get('rows').val() || 50);
  },
