...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    <sch:rule context="pds:Checksum_Manifest">
      ...
      <sch:assert test="if (pds:encoding_type) then pds:encoding_type = ('Character') else true()">
  ...
  <sch:pattern>
    <sch:rule context="//pds:Citation_Information/pds:description">
      <sch:assert test="string-length(translate(., ' ', '')) &gt;= 50 and string-length(translate(., ' ','')) &lt;= 5000">
        The description in Citation_Information must be greater than 50 and less than 5000 bytes (not counting spaces).</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
  ...
  <sch:pattern>
    <sch:rule context="pds:Display_2D_Image">
      <sch:assert test="if (pds:line_display_direction) then pds:line_display_direction = ('Down', 'Up') else true()">
        The attribute pds:line_display_direction must be equal to one of the following values 'Down', 'Up'.</sch:assert>
      <sch:assert test="if (pds:sample_display_direction) then pds:sample_display_direction = ('Right') else true()">
        The attribute pds:sample_display_direction must be equal to the value 'Right'.</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
    <sch:rule context="pds:Document_File">
      <sch:assert test="if (pds:document_standard_id) then pds:document_standard_id = ('7-Bit ASCII', 'Encapsulated Postscript', 'GIF', 'HTML 2.0', 'HTML 3.2', 'HTML 4.0', 'HTML 4.01', 'JPEG', 'LaTEX', 'Microsoft Word', 'PDF', 'PDF/A', 'PNG', 'Postscript', 'Rich Text', 'TIFF', 'UTF-8') else true()">
        The attribute pds:document_standard_id must be equal to one of the following values '7-Bit ASCII', 'Encapsulated Postscript', 'GIF', 'HTML 2.0', 'HTML 3.2', 'HTML 4.0', 'HTML 4.01', 'JPEG', 'LaTEX', 'Microsoft Word', 'PDF', 'PDF/A', 'PNG', 'Postscript', 'Rich Text', 'TIFF', 'UTF-8'.</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
  ...
  <sch:pattern>
    <sch:rule context="pds:Encoded_Binary">
      ...
      <sch:assert test="if (pds:encoding_type) then pds:encoding_type = ('Binary') else true()">
  ...
  <sch:pattern>
    <sch:rule context="pds:Identification_Area">
      ...
      <sch:assert test="if (pds:information_model_version) then pds:information_model_version = ('0.3.1.0.b') else true()">
        The attribute pds:information_model_version must be equal to the value '0.3.1.0.b'.</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
    <sch:rule context="/*">
  ...
  <sch:pattern>
    <sch:rule context="pds:Primary_Result_Summary">
      <sch:assert test="if (pds:data_regime) then pds:data_regime = ('Dust', 'Electric Field', 'Electrons', 'Far Infrared', 'Gamma Ray', 'Infrared', 'Ions', 'Magnetic Field', 'Microwave', 'Millimeter', 'Near Infrared', 'Particles', 'Pressure', 'Radio', 'Sub-Millimeter', 'Temperature', 'Ultraviolet', 'Visible', 'X-Ray') else true()">
      ...
      <sch:assert test="if (pds:type) then pds:type = ('Altimetry', 'Astrometry', 'Count', 'E/B-Field Vectors', 'Gravity Model', 'Image', 'Magnetometry', 'Map', 'Null Result', 'Occultation', 'Photometry', 'Polarimetry', 'Radiometry', 'Shape Model', 'Spectrum') else true()">
        The attribute pds:type must be equal to one of the following values 'Altimetry', 'Astrometry', 'Count', 'E/B-Field Vectors', 'Gravity Model', 'Image', 'Magnetometry', 'Map', 'Null Result', 'Occultation', 'Photometry', 'Polarimetry', 'Radiometry', 'Shape Model', 'Spectrum'.</sch:assert>
      ...
      <sch:assert test="if (pds:processing_level_id) then pds:processing_level_id = ('Calibrated', 'Derived', 'Partially Processed', 'Raw', 'Telemetry') else true()">
  ...
  <sch:pattern>
    <sch:rule context="pds:Product_Document/pds:Context_Area/pds:Observing_System/pds:Observing_System_Component/pds:Internal_Reference">
      <sch:assert test="every $ref in (pds:reference_type) satisfies $ref = ('document_to_instrument', 'document_to_instrument_host')">
        The attribute reference_type must be set to one of the following values 'document_to_instrument', 'document_to_instrument_host'.</sch:assert>
    </sch:rule>
  </sch:pattern>
  <sch:pattern>
